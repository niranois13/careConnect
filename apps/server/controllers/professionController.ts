...
import { ProfessionSchema, CustomProfessionSchema } from '../../../packages/schemas/src/profession.schemas.ts';
...
export async function adminCreateProfessions(req: Request, res: Response) {
  try {
    const professionData = ProfessionSchema.parse(req.body);
    ...
    const newProfession = await prisma.profession.create({
      data: {
        ...
        isCustomProfessionApproved: professionData.isCustomProfessionApproved,
      },
      include: {
        professionals: true,
      }
    ...
    isCustomProfessionApproved: newProfession.isCustomProfessionApproved,
  };
...
export async function proCreateProfessions(req: Request, res: Response) {
  try {
    const parsedProfession = CustomProfessionSchema.parse(req.body);
